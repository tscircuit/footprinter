...
test("dip16", () => {
    const circuitJson = fp.string("dip16").circuitJson();
    const svgContent = convertCircuitJsonToPcbSvg(circuitJson);
...
test("dip10_w4.00mm_p2.65mm", () => {
    const circuitJson = fp
        ...
        .circuitJson();
    const svgContent = convertCircuitJsonToPcbSvg(circuitJson);
    expect(svgContent).toMatchSvgSnapshot(import.meta.path, "dip10_w4.00mm_p2.65mm");
});
test("dip4", () => {
...
test("dip14_w7.62mm_p2.54mm", () => {
    const circuitJson = fp
        ...
        .circuitJson();
    const svgContent = convertCircuitJsonToPcbSvg(circuitJson);
    expect(svgContent).toMatchSvgSnapshot(import.meta.path, "dip14_w7.62mm_p2.54mm");
});
test("dip_0.1in", () => {
    const circuitJson = fp
        .string("dip_0.1in")
