...
test("qfp48_w14_p1mm_startingpin(topside,leftpin)", () => {
    const soup = fp
        ...
        .circuitJson();
    const svgContent = convertCircuitJsonToPcbSvg(soup);
    expect(svgContent).toMatchSvgSnapshot(import.meta.path, "qfp48_w14_p1mm_startingpin(topside,leftpin)");
});
test("qfp80_w14_h14_p0.65mm", () => {
    const soup = fp
        ...
        .circuitJson();
    const svgContent = convertCircuitJsonToPcbSvg(soup);
    expect(svgContent).toMatchSvgSnapshot(import.meta.path, "qfp80_w14_h14_p0.65mm_startingpin(topside,leftpin)");
});
test("qfp52", () => {
