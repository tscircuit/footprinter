...
type CourtyardElement =
  ...
  | PcbCourtyardCircle
  | PcbCourtyardPolygon
  | (Omit<PcbCourtyardCircle, "radius"> & {
      radius?: number;
      diameter: number;
    });
...
export async function compareFootprinterVsKicad(
  ...
  if (!res.ok)
    throw new Error(
      `Failed to fetch ${normalizedFootprintName}: ${res.status}`,
    );
  ...
  const kicadMaxY = Math.max(
    ...kicadAlignmentBounds.map((bound) => bound.maxY),
  );
  const kicadMinY = Math.min(
    ...kicadAlignmentBounds.map((bound) => bound.minY),
  );
