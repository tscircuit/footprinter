...
test("sod323fl", () => {
  const circuitJson = fp.string("sod323fl").circuitJson()
  const svgContent = convertCircuitJsonToPcbSvg(circuitJson, {
    showCourtyards: true,
  })
  expect(svgContent).toMatchSvgSnapshot(import.meta.path, "sod323fl")
})

test("sod323fl fabrication keeps the outline width but uses a narrower symbol", () => {
  const circuitJson = fp.string("sod323fl").circuitJson()
  ...
  expect(arrowWidth).toBeLessThan(outlineWidth * 0.35)
})
