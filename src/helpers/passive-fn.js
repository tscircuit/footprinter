...
const metricMap = footprintSizes.reduce((acc, s) => {
    acc[s.metric] = s;
    return acc;
}, {});
const imperialMap = footprintSizes.reduce((acc, s) => {
    acc[s.imperial] = s;
    return acc;
}, {});
...
const deriveXFromH = (h) => 0.079 * h ** 2 + 0.94 * h - 0.009;
const deriveZFromW = (w) => 1.09 * w + 0.6;
const deriveGFromW = (w) => 0.59 * w - 0.31;
const deriveCFromW = (w) => -0.01 * w ** 2 + 0.94 * w + 0.03;
