import { length, type AnySoupElement, type PcbSilkscreenPath } from "circuit-json"
import { z } from "zod"
...
export const gbj = (
  ...
  positions.forEach((x, index) => {
    ...
    if (pinNumber === 1) {
      pads.push(platedHoleWithRectPad(
        pinNumber, x, 0,
        params.id, params.od, params.od
      ))
    } else {
      pads.push(platedhole(
        pinNumber, x, 0,
        params.id, params.od
      ))
    }
  ...
  const silkBody: PcbSilkscreenPath = {
    ...
    stroke_width: 0.1,
    route: [
        { x: -params.w / 2, y: -params.h / 2 }, { x: params.w / 2, y: -params.h / 2 },
        { x: params.w / 2, y: params.h / 2 }, { x: -params.w / 2, y: params.h / 2 },
  ...
  const silkPlus: PcbSilkscreenPath = {
     ...
     stroke_width: 0.1,
     route: [
         { x: x1 - 2, y: 0 }, { x: x1 - 1, y: 0 },
         { x: x1 - 1.5, y: -0.5 }, { x: x1 - 1.5, y: 0.5 } 
     ]
  }
