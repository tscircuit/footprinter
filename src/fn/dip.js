...
export const extendDipDef = (newDefaults) => z
    .object({
    fn: z.string(),
...
export const getCcwDipCoords = (pinCount, pn, w, p) => {
    /** pin height */
    const ph = pinCount / 2;
    const isLeft = pn <= ph;
...
export const dip = (raw_params) => {
    ...
    for (let i = 0; i < parameters.num_pins; i++) {
        const { x, y } = getCcwDipCoords(parameters.num_pins, i + 1, parameters.w, parameters.p ?? 2.54);
