...
const createManualDiodeFabricationNotes = (params: {
  ...
  const arrowBaseRatio = clamp(0.24 - tightPitchBoost * 0.03, 0.2, 0.24)
  const cathodeBarRatio = clamp(0.16 - tightPitchBoost * 0.02, 0.12, 0.16)
  ...
  const symbolHalfHeight = clamp(
    Math.min(
      provisionalBodyHeight * (0.26 + tightPitchBoost * 0.05),
      usableSymbolWidth * (0.31 + tightPitchBoost * 0.08),
    ),
    0.11,
    provisionalBodyHeight * (0.35 + tightPitchBoost * 0.04),
  )
