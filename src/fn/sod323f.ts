...
const createManualDiodeFabricationNotes = (params: {
  ...
  const symbolLeadInsetX = clamp(padDistance * 0.16, 0.14, 0.36)
  const symbolLeadLeftX = leadLeftX + direction * symbolLeadInsetX
  const symbolLeadRightX = leadRightX - direction * symbolLeadInsetX
  ...
  const arrowBaseRatio = clamp(0.34 - tightPitchBoost * 0.02, 0.3, 0.34)
  const cathodeBarRatio = clamp(0.28 - tightPitchBoost * 0.02, 0.24, 0.28)
  const arrowBaseX =
