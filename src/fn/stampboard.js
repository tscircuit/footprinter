...
export const stampboard = (raw_params) => {
    ...
    if (params.top) {
        ...
        for (let i = 0; i < params.top; i++) {
            ...
            if (params.innerhole) {
                holes.push(platedhole(i +
                    1 +
                    params.left +
                    params.right +
                    (params.bottom ?? 0) +
                    totalPadsNumber, xoff + i * params.p, height / 2, innerDiameter, outerDiameter));
                holes.push(platedhole(i +
                    1 +
                    params.left +
                    params.right +
                    (params.bottom ?? 0) +
                    totalPadsNumber * 2, xoff + i * params.p, height / 2 - params.innerholeedgedistance, innerDiameter, outerDiameter));
            }
        }
    }
    if (params.bottom) {
