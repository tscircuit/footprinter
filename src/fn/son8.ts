...
export const son8_def = z.object({
  fn: z.string(),
  ...
  has_ep: z.boolean().default(false),
})

export const son8 = (
  raw_params: z.input<typeof son8_def>,
): { circuitJson: AnyCircuitElement[]; parameters: any } => {
  if (raw_params.string && raw_params.string.includes("_ep")) {
    raw_params.has_ep = true
  }

  const parameters = son8_def.parse(raw_params)
  ...
  for (let i = 0; i < parameters.num_pins; i++) {
    const { x, y } = getSon8PadCoord(parameters.num_pins, i + 1, w, p)
    pads.push(rectpad(i + 1, x, y, pl, pw))
  }

  if (parameters.has_ep) {
  ...
  const pin1Position = getSon8PadCoord(
    parameters.num_pins,
    1,
    silkscreenBoxWidth,
    p,
