...
const getTriangleDir = (x, y, side, triangleHeight = 1, triangleWidth = 0.6) => {
...
export const stampreceiver = (raw_params) => {
    ...
    if (params.left) {
        ...
        for (let i = 0; i < params.left; i++) {
            if (i === 0) {
                routes = getTriangleDir(-params.w / 2 - params.pl / 2, yoff - i * params.p, "left");
            }
            ...
            padIndex++;
            params.innerhole &&
                holes.push(platedhole(padIndex + totalPadsNumber, -params.w / 2 + params.innerholeedgedistance, yoff - i * params.p, innerDiameter, outerDiameter));
        }
    }
    ...
    if (params.bottom) {
        ...
        for (let i = params.bottom - 1; i >= 0; i--) {
            ...
            padIndex++;
            params.innerhole &&
                holes.push(platedhole(padIndex + totalPadsNumber, xoff - i * params.p, -height / 2 + params.innerholeedgedistance, innerDiameter, outerDiameter));
        }
    }
