...
export const jst = (
  ...
  // Match variant-prefixed formats: jst_ph_4, jst_sh_6, jst_zh_3, etc.
  const variantMatch = str.match(/(?:^|_)(?:ph|sh|zh)[_-]?(\d+)(?:_|$)/)
  // Match trailing number after last underscore: jst_ph_4 -> 4
  const trailingMatch = str.match(/_(\d+)$/)
  if (match && match[1]) {
  ...
  if (typeof numPins !== "number" && variantMatch && variantMatch[1]) {
    const parsed = Number.parseInt(variantMatch[1], 10)
    if (!Number.isNaN(parsed)) {
      numPins = parsed
    }
  }
