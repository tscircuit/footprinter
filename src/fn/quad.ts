...
export const getQuadCoords = (params: {
   ...
   * QFP/TQFP/LQFP (legsoutside=true): pad center is pl/2 outside the body edge.
   * QFN/MLP (legsoutside=false): pad center is at the body edge per IPC-7351B
   * (leads flush with package edge, extending outward to the solderable toe).
   */
  const pcdfe = legsoutside ? pl / 2 : 0

  /** small inward nudge — only applies to legsoutside packages to match KiCad */
