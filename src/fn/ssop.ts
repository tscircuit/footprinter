...
export const ssop = (
  ...
  const padYExtent = ((parameters.num_pins / 2 - 1) * parameters.p) / 2
  const courtyardWidthMm = padXExtent * 2 + 0.5
  const courtyardHeightMm = (padYExtent + parameters.pw / 2) * 2 + 0.89
