...
export const son8ep = (
  raw_params: z.input<typeof son8ep_def>,
): { circuitJson: AnyCircuitElement[]; parameters: any } => {
  const parameters = son8ep_def.parse(raw_params)
  ...
  for (let i = 0; i < parameters.num_pins; i++) {
    const { x, y } = getSon8epPadCoord(parameters.num_pins, i + 1, w, p)
  ...
  const pin1Position = getSon8epPadCoord(
    parameters.num_pins,
    1,
    silkscreenBoxWidth,
    p,
...
// Get coordinates for SON-8-EP pads
export const getSon8epPadCoord = (
  num_pins: number,
  pn: number,
  w: number,
  p: number,
